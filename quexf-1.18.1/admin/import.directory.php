...
if ($p)
{
	...
	if (is_process_killed($p))
	{
		...
		print "<p><a href='?force_kill'>" . T_("Mark the process as killed (i.e. when the server is rebooted)"). "</a></p>";
	}
	else
...
else
{
	...
	{
		echo "<h1>dir is set</h1>";
	    $dir = $_POST['dir'];
		import_directory($dir);
	}

	?>	
	<h1><?php echo T_("Import Files"); ?></h1>
	<form enctype="multipart/form-data" action="?" method="post">
	    <p>
        <?php echo T_("Choose directory with pdf files from your local disk"); ?><br><br>

            <label for="importing pdfs">Choose Directory: </label><input name="dir" type="file" webkitdirectory directory multiple />
        </p>
